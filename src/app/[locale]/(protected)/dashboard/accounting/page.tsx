...
export default function AccountingPage() {
  ...
  // 卡片選中狀態管理
  const [selectedCard, setSelectedCard] = useState<'tax' | 'audit' | 'bookkeeping' | 'consulting'>('tax');
  ...
  const cards = [
    { id: 'tax', title: '稅務申報' },
    { id: 'audit', title: '財務稽核' },
    { id: 'bookkeeping', title: '帳務處理' },
    { id: 'consulting', title: '會計諮詢' },
  ];
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      uploadFormData.append('category', 'accounting-tax');
                    ...
                    >
                      <div className="flex items-center justify-center h-full">
                        <h3 className={`text-lg font-semibold ${
                          selectedCard === card.id ? 'text-teal-900' : 'text-gray-900'
                        }`}>
                          {card.title}
                        </h3>
                      </div>
