...
// 建立銷售支援記錄
export async function createSalesSupportRecord(input: CreateSalesSupportInput) {
  try {
    // 驗證輸入資料
...
// 取得銷售支援記錄列表
export async function getSalesSupportRecords(category?: string, item?: string) {
  try {
    let whereClause = '';
    const params: any[] = [];
    let paramIndex = 1;
